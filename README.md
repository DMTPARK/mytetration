...
- 확대를 원하는 부분의 좌표찾기는 rendering이 끝났을때 화면에 뜨는 'Figure 1'창에서 가능합니다. 해당창을 클릭하고 마우스로 이미지를 훑어보면, 우측하단에 마우스커서 위치의 (x,y)좌표가 표시됩니다. 원하는 부분의 좌표를 `x0`, `y0`에 입력하고, 확대정도는 `eps`변수를 통해 정하면 출력범위가 재설정 됩니다. `eps=1e-1`은 10<sup>-1</sup>를 뜻하며, 1.234&times;10<sup>-12</sup>를 입력하고 싶다면, `eps=1.234e-12`로 수정하면 됩니다.
- 이미지 비율과 회전
